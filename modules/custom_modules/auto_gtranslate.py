...
async def set_language(_, message: Message):
    ...
    if len(message.command) < 2:
        await message.edit(
            f"Usage: <code>{prefix}setglang [language_code]</code>\n"
            "<b>Example:</b>\n"
            "<code>setglang ru</code> to set Russian."
        )
        return
...
async def language_status(_, message: Message):
    ...
    if command_text == f"{prefix}glang":
        ...
        if lang_code:
            await message.edit(f"<b>Language is set to</b> <code>{lang_code}</code>.")
        else:
            await message.edit("No language set. Use <code>setglang</code> to set one.")
    elif command_text == f"{prefix}glang off":
        result = db.remove("custom.gtranslate", chat_id)
        if result:
            await message.edit("<b>Auto-translation has been turned off.</b>")
        else:
            await message.edit("<b>Auto-translation is disabled.</b>")
    else:
        await message.edit(f"<b>Usage:</b> <code>{prefix}glang</code> to check the language or <code>{prefix}glang off</code> to turn off auto-translation.")
